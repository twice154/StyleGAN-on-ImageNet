...
class SpatialModulationNetwork(torch.nn.Module):
    def __init__(self,
        ...
        for idx in range(len(block_resolutions)*2):
            # Just convolution.
            if idx == 0:
                ...
                mod_channels = 1
                layer = Conv2dLayer(in_channels, out_channels, kernel_size=3, bias=False, activation='lrelu', up=1)
            ...
            # Just convolution.
                ...
                dimensions = block_resolutions[int(idx/2)]
                in_channels = int(int(w_dim / (dimensions * dimensions)) / (2**int(idx/2)))
                out_channels = in_channels
                mod_channels = 1
                layer = Conv2dLayer(in_channels, out_channels, kernel_size=3, bias=False, activation='lrelu', up=1)
                setattr(self, f'conv{idx}', layer)
            ...
            # Upsampling before convolution.
                ...
                dimensions = block_resolutions[int(idx/2)]
                in_channels = int(int(w_dim / (dimensions * dimensions)) / (2**(int(idx/2)-1)))
                out_channels = int(in_channels / 2)
                mod_channels = 1
                layer = Conv2dLayer(in_channels, out_channels, kernel_size=3, bias=False, activation='lrelu', up=2)
                setattr(self, f'conv{idx}', layer)
                mod_layer = Conv2dLayer(out_channels, mod_channels, kernel_size=1, bias=False, activation='linear', up=1)
                setattr(self, f'mod{idx}', mod_layer)
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False):
        ...
        for idx in range(len(self.block_resolutions)*2):
...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(self, c, x, img, ws, force_fp32=False, fused_modconv=None, **layer_kwargs):
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, c, ws, **block_kwargs):
        block_ws = []
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            block = getattr(self, f'b{res}')
            x, img = block(c, x, img, cur_ws, **block_kwargs)
        return img
...
class Generator(torch.nn.Module):
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs):
        ...
        spws = self.spatial(z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff)
        img = self.synthesis(c, ws, **synthesis_kwargs)
        return img
